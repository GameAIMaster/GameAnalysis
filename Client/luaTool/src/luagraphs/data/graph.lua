...
function graph:removeEdge(v, w)
    if self.vertexList:contains(v) and self.vertexList:contains(w) then

        local adj_v = self.adjList[v]
        local adj_w = self.adjList[w]
        for k = 1,adj_v:size() do
            local e = adj_v:get(k)
            if e:other(v) == w then
                adj_v:removeAt(k)
                break
